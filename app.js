...
const LAST_SECTION_KEY = "ccx_last_section_v9";
let state = { users: [], posts: [], currentUser: null };
let editPostId = null, currentChatPostId = null;
...
const startListeners = () => {
    ...
    // 2. Listen to Users (Profile Sync)
    onSnapshot(collection(db, "users"), (snap) => {
        state.users = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
...
const renderInbox = () => {
    ...
    // [LOGIC] Count total unread messages from OTHERS
    let totalUnread = 0;
    items.forEach(p => {
