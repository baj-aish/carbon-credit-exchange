...
import {
  ...
  updateDoc, arrayUnion, query, orderBy, onSnapshot, setDoc, deleteDoc
...
const getEl = i => typeof i === "string" ? qs(i) : i;
const show = i => getEl(i)?.classList.remove("hidden");
const hide = i => getEl(i)?.classList.add("hidden");
const toggle = (i, c) => c ? show(i) : hide(i);

const LAST_SECTION_KEY = "ccx_FINAL_CHAT_FIX";
let state = { users: [], posts: [], currentUser: null };
let editPostId = null;
...
// ==========================================
// 4. CHAT LOGIC (FROM app.js 1 — WORKING)
// ==========================================
window.openChat = async (postId) => {
  ...
  qs("chatPostTitle").textContent = post.title;
  show("chatModal");
  show("chatForm");
  renderChatMessages(post);

  // mark seen
  const msgs = post.chatMessages || [];
  if (msgs.some(m => m.from !== state.currentUser.name && !m.seen)) {
    const updated = msgs.map(m =>
      m.from !== state.currentUser.name ? { ...m, seen: true } : m
    );
    await updateDoc(doc(db, "posts", postId), { chatMessages: updated });
  }
};

const renderChatMessages = (post) => {
  const box = qs("chatMessages");
  box.innerHTML = (post.chatMessages || []).map(m => {
    const isMe = m.from === state.currentUser.name;
    return `
      <div class="flex ${isMe ? "justify-end" : "justify-start"}">
        <div class="px-3 py-1.5 rounded text-xs max-w-[80%]
          ${isMe ? "bg-emerald-600 text-white" : "bg-slate-800 text-slate-300"}">
          <div class="text-[9px] opacity-60">${m.from}</div>
          ${m.text}
        </div>
      </div>`;
  }).join("");
...
on(qs("chatForm"), "submit", async e => {
  e.preventDefault();
  const text = qs("chatInput").value.trim();
  if (!text || !currentChatPostId) return;
...
// ==========================================
// 5. INBOX (USES post.chatMessages)
// ==========================================
const renderInbox = () => {
  if (!state.currentUser) {
    toggle("inboxIndicator", false);
    qs("inboxList").innerHTML = `<p class="text-slate-400 text-sm">Login required.</p>`;
    return;
  }

  const myName = state.currentUser.name;
  const threads = state.posts.filter(p =>
    p.chatMessages?.some(m => m.from === myName || p.user === myName)
  );

  let unread = 0;
  threads.forEach(p => {
    unread += p.chatMessages.filter(m => m.from !== myName && !m.seen).length;
  });

  toggle("inboxIndicator", unread > 0);

  qs("inboxList").innerHTML = threads.length
    ? threads.map(p => `
      <div onclick="openChat('${p.id}')"
        class="bg-slate-900 p-3 rounded border border-slate-700 cursor-pointer">
        <div class="font-bold text-emerald-100">${p.title}</div>
        <div class="text-xs text-slate-400">
          ${p.chatMessages.at(-1).from}: ${p.chatMessages.at(-1).text}
        </div>
      </div>`).join("")
