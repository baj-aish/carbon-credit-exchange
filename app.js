...
// // const LAST_SECTION_KEY = "ccx_FINAL_MERGED_v501";
...
// //     if(chatBtn) {
// //         e.preventDefault();
// //         const postId = chatBtn.dataset.id;
...
// // // ==========================================
// // const htmlPost = (p) => {
// //     // CRITICAL FIX: Check using IDs if available, else name
// //     let isMine = false;
// //     if (state.currentUser) {
// //         if (p.ownerId) isMine = p.ownerId === state.currentUser.id;
// //         else isMine = p.user === state.currentUser.name;
// //     }

// //     const badge = isMine ? `<span class="bg-emerald-500 text-slate-900 text-[10px] font-bold px-2 py-0.5 rounded ml-2">CREATED BY YOU</span>` : '';
// //     // FIXED: Only render button if !isMine
...
// // Delete Button Listener in Edit Form (ADDED THIS BLOCK)
// on(qs("deleteEditBtn"), "click", async () => {
//     if(!editPostId) return;
//     if(confirm("Delete this listing permanently?")) {
//         await deleteDoc(doc(db, "posts", editPostId));
...
const LAST_SECTION_KEY = "ccx_last_section_v12_private";
// Added 'chats' to state for private messaging
let state = { users: [], posts: [], chats: [], currentUser: null };
let editPostId = null, currentChatId = null, chatListenerUnsubscribe = null;
...
const startListeners = () => {
    ...
    onSnapshot(collection(db, "users"), (snap) => {
        ...
        if (auth.currentUser) {
            const me = state.users.find(u => u.id === auth.currentUser.uid);
            if (me) {
                // If this is a fresh load or update
                if (!state.currentUser || state.currentUser.id !== me.id) {
                    state.currentUser = me;
                    updateAuthUI();
                    subscribeToMyChats(me.id); // <--- START PRIVATE CHAT LISTENER
                    renderFeed(); 
                }
            }
        }
...
// New: Listen only to chats where I am a participant
const subscribeToMyChats = (userId) => {
    if (chatListenerUnsubscribe) chatListenerUnsubscribe(); // Stop old listener

    const q = query(collection(db, "chats"), where("participants", "array-contains", userId));
    
    chatListenerUnsubscribe = onSnapshot(q, (snap) => {
        state.chats = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        renderInbox(); // Update Inbox UI
        
        // If a chat window is currently open, update its messages live
        if (currentChatId && !qs("chatModal").classList.contains("hidden")) {
...
on(qs("loginForm"), "submit", async e => {
  ...
  try {
    ...
    state.currentUser = { id: userDoc.id, ...userDoc.data() };
    updateAuthUI();
    subscribeToMyChats(state.currentUser.id); // Start listener immediately
    renderFeed();
    hide("loginModal");
    showSection("feed");
  } catch (err) { alert(err.message); }
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        // Handled by User Listener above to ensure full profile data
    } else {
        state.currentUser = null;
        state.chats = [];
        updateAuthUI(); renderFeed(); renderInbox();
...
const renderInbox = () => {
    ...
    // 2. Count Unread (Total across all chats)
    let totalUnread = 0;
    state.chats.forEach(chat => {
        totalUnread += chat.messages.filter(m => m.senderId !== myId && !m.seen).length;
    });
    toggle("inboxIndicator", totalUnread > 0);
    ...
    // Sort by last update
    state.chats.sort((a,b) => (b.lastUpdated || 0) - (a.lastUpdated || 0));
    
    list.innerHTML = state.chats.map(chat => {
        return htmlInboxItem(chat, myId);
    }).join("");
};
...
// Open Chat Logic: Finds or Creates a PRIVATE chat document
window.openChat = async (postId) => {
    if(!requireLogin()) return;
    ...
    const myId = state.currentUser.id;
    // Find owner ID based on name (Legacy support) or if stored
    const owner = state.users.find(u => u.name === post.user);
    if(!owner) return alert("Seller not found.");
    ...
    // We look for a chat in state.chats that matches the criteria
...
const renderChatMessages = (chat) => {
    ...
    box.innerHTML = (chat.messages||[]).map(m => {
        const isMe = m.senderId === myId;
        return `<div class="flex ${isMe?'justify-end':'justify-start'}"><div class="px-2 py-1 rounded mb-1 text-xs max-w-[80%] ${isMe?'bg-emerald-600 text-white':'bg-slate-800 text-slate-300'}"><div class="font-bold opacity-50 text-[9px]">${m.senderName}</div>${m.text}<div class="text-[9px] opacity-60 text-right">${isMe && m.seen ? 'Seen' : ''}</div></div></div>`;
    }).join("");
    box.scrollTop = box.scrollHeight;

    // Mark messages as seen if they are not mine
    const needsUpdate = chat.messages.some(m => m.senderId !== myId && !m.seen);
    if(needsUpdate) {
        const updatedMsgs = chat.messages.map(m => {
            if(m.senderId !== myId) return { ...m, seen: true };
            return m;
        });
        updateDoc(doc(db, "chats", chat.id), { messages: updatedMsgs });
    }
...
window.viewAdminChat = (pid) => { /* Admin view logic can be adapted similarly if needed */ };
...
on(qs("chatForm"), "submit", async e => {
    ...
    // Add message to the specific chat document
    await updateDoc(doc(db, "chats", currentChatId), { 
        messages: arrayUnion({ 
...
const htmlPost = (p) => {
    ...
    // Hide chat button if it's my post
    const chatButton = isMine ? '' : `<div class="mt-auto"><button onclick="window.openChat('${p.id}')" class="w-full py-2 bg-slate-800 text-xs rounded border border-slate-600 relative hover:bg-slate-700">💬 Chat</button></div>`;

    return `<div class="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden shadow flex flex-col"><img src="${p.image}" class="w-full h-44 object-cover"><div class="p-3 flex flex-col flex-1"><div class="flex justify-between items-start mb-1"><h3 class="font-bold text-sm truncate flex-1">${p.title}</h3>${badge}</div><p class="text-[10px] text-slate-500 mb-2">By ${p.user} • ${new Date(p.createdAt).toLocaleDateString()}</p><p class="text-xs text-slate-400 mb-2 truncate">${p.desc}</p><div class="flex justify-between text-[11px] mb-2"><span class="text-emerald-300 bg-emerald-900/30 px-2 py-0.5 rounded-full">${p.credits} Credits</span><span class="text-white font-bold">₹${p.price}</span></div>${chatButton}</div></div>`;
};
const htmlListRow = (p) => `<div class="bg-slate-900 p-2 border border-slate-700 rounded mb-2 flex justify-between text-xs items-center"><span>${p.title}</span><button class="text-emerald-400 underline" onclick="window.editPost('${p.id}')">Edit</button></div>`;

const htmlInboxItem = (chat, myId) => {
    const msgs = chat.messages || [];
    const last = msgs.length > 0 ? msgs[msgs.length - 1] : { text: "No messages yet", senderName: "" };
    const unreadCount = msgs.filter(m => m.senderId !== myId && !m.seen).length;
    
    // Find the OTHER person's name
    const otherName = chat.participantNames.find(name => name !== state.currentUser.name) || "User";

    return `<div onclick="window.openExistingChat('${chat.id}')" class="bg-slate-900 p-3 rounded-lg border border-slate-700 cursor-pointer flex justify-between items-center hover:bg-slate-800"><div><div class="text-sm font-bold text-emerald-100 flex items-center gap-2">${chat.postTitle} <span class="text-[10px] text-slate-400 font-normal">w/ ${otherName}</span> ${unreadCount > 0 ? `<span class="bg-red-500 text-white text-[9px] px-1.5 rounded-full">${unreadCount}</span>` : ''}</div><div class="text-xs text-slate-400 truncate max-w-[200px]">${last.senderName}: ${last.text}</div></div><div class="text-xs text-emerald-500">Open</div></div>`;
};
