...
const LAST_SECTION_KEY = "ccx_FINAL_MERGED_v300";
let state = { users: [], posts: [], chats: [], currentUser: null };
let editPostId = null, currentChatId = null, chatUnsubscribe = null;
...
const startListeners = () => {
    ...
    onSnapshot(collection(db, "users"), (snap) => {
        ...
        if (auth.currentUser) {
            const me = state.users.find(u => u.id === auth.currentUser.uid);
            if (me) {
                // If user just loaded or changed, update state & start inbox
                if (!state.currentUser || state.currentUser.id !== me.id) {
                    state.currentUser = me;
                    updateAuthUI();
...
// This logic comes from app.js(1) which was working
document.addEventListener("click", (e) => {
    const btn = e.target.closest(".chat-btn");
    if(btn) {
        e.preventDefault();
        const postId = btn.dataset.id; // Reads data-id from HTML
